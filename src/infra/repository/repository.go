...
func create(entity interface{}, value interface{}) (int64, error) {
	...
	var id int64
	if err := postgres.QueryRow(query.String()).Scan(&id); err != nil {
		return 0, err
	}
...
func delete(tableName string, id int64) error {
	...
	if _, err := postgres.Exec(query, id); err != nil {
		return err
	}
...
func _getQueryFields(val reflect.Value, value interface{}) (strings.Builder, strings.Builder) {
	...
	for i := 0; i < totalFields; i++ {
		if val.Type().Field(i).Tag.Get("postgres") == "id" && _getFieldValue(value, val.Type().Field(i)) == "0" {
			continue
		}

		columns.WriteString(val.Type().Field(i).Tag.Get("postgres"))
...
func _getQueryUpdate(entity interface{}) string {
	...
	var id string
	for i := 0; i < totalFields; i++ {
		if val.Type().Field(i).Tag.Get("postgres") == "id" {
			id = _getFieldValue(entity, val.Type().Field(i))
			continue
		}

		query.WriteString(val.Type().Field(i).Tag.Get("postgres"))
		query.WriteString("=")
