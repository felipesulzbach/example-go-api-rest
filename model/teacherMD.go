...
func (db *DB) InsertTeacher(entityy entity.Teacher) (int64, error) {
  ...
  err := db.QueryRow(sqlStatement, entityy.Person.ID, entityy.Course.ID).Scan(&returnedID)
  if err != nil {
    return 0, err
  }
...
func (db *DB) UpdateTeacher(entityy entity.Teacher) error {
  sqlStatement := "UPDATE GO_TST.teacher SET course_id=$2 WHERE person_id=$1"
  _, err := db.Exec(sqlStatement, entityy.Person.ID, entityy.Course.ID)
  if err != nil {
    return err
  }
